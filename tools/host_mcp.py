...
class ToolResult(BaseModel):
    """B2 — return this (not a bare ``dict``) so FastMCP emits UNWRAPPED structuredContent and the
    fast-rlm ``mcp_call`` proxy hands the agent a REAL dict (its ``r["valid"]`` / ``r.get(...)`` /
    ``isinstance(r, dict)`` then work). Dict-compatible so host-side callers can still index it."""
...
# Skills loadable on demand via load_skill(topic=...). (v4: this map was MISSING in v3,
# so load_skill raised NameError and the freeform path the prompt advertised was dead.)
_SKILLS = {
    "freeform": ROOT / "skills" / "freeform.md",
    "assembly": ROOT / "skills" / "assembly.md",
    "verify": ROOT / "meshlib_kb_pack" / "skills" / "meshlib_verify.md",
}
...
@mcp.tool()
def validate_plan(plan: dict) -> "ToolResult":
    ...
    try:
        from schemas.geometry_plan import GeometryPlan, PRIMITIVES_REGISTRY, MODIFIER_TYPES
        valid_types = sorted(list(PRIMITIVES_REGISTRY.keys()) + ["custom"] + list(MODIFIER_TYPES))
    except Exception as e:
        return ToolResult(**{"valid": False,
                             "errors": [{"location": "", "message": f"validator load failed: {e}"}],
                             "valid_primitive_types": []})
    try:
        GeometryPlan(**plan)
        return ToolResult(**{"valid": True, "errors": [], "valid_primitive_types": valid_types})
    except Exception as e:
        errors = []
        if hasattr(e, "errors"):
            for err in e.errors():
        ...
        return ToolResult(**{"valid": False, "errors": errors, "valid_primitive_types": valid_types})


# --- primitive library tool (host-side copy so the RLM can browse it via MCP) ---
@mcp.tool()
def merge_subplans(parts: list, connections: list = None,
                   assembly_kind: str = "single_solid", title: str = None) -> dict:
    """Deterministically stitch child sub-plans into ONE GeometryPlan (recursive assembly).

    Does the single-right-answer bookkeeping you should not do by hand: namespaces step
    names, rewires intra-part `attach` references, renumbers sequence_ids 1..N, tags each
    step's `part`, and applies the cross-part `connections` you specify. The geometry under
    it still flows through the deterministic mate kernel.

    Args:
      parts: [{"name": str, "plan": <sub-plan dict with primitives_sequence>}, ...]
      connections: optional [{"from": "<part>"|"<part>.<step>", "to": "<part>"|"<part>.<step>",
                   "at": <anchor>, "my_anchor"?: <anchor>, "gap"?: float, "offset"?: [x,y,z]}, ...].
                   A bare "<part>" means that part's seed (first) step.
      assembly_kind: "single_solid" (fuse) or "assembly" (keep parts separate).
      title: optional plan title.
    Returns: a merged GeometryPlan dict (overall_dimensions is provisional — reconcile it
    against the measured bbox after build_verify_render).
    """
    import sys as _sys
    if str(ROOT) not in _sys.path:
        _sys.path.insert(0, str(ROOT))
    from cad_kernel.merge import merge_subplans as _merge
    try:
        return _merge(parts, connections=connections, assembly_kind=assembly_kind, title=title)
    except Exception as e:
        return {"error": f"merge_subplans failed: {e}"}
