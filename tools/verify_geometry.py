...
def verify_geometry(
    ...
    """
    Vision-LLM judge: sends the prompt, OCCT kernel metrics, and a three-view
    render to Gemini and returns a structured verdict.
    ...
    Returns:
        dict[str, Any]: {"passed": bool, "feedback": str, "failure_category": str | None, "render_png": str}

    Never raises. If GEMINI_API_KEY is missing/placeholder -> mock pass. On a
    judge/parse/transport failure -> {"passed": False, "failure_category": "verifier_miss", ...}
    so the outer loop routes to refinement instead of crashing.
    """
    import json
    import os
    import re
    ...
    model = os.getenv("GEMINI_JUDGE_MODEL") or os.getenv("JUDGE_MODEL") or "gemini-3.5-flash"
    max_tokens = int(os.getenv("JUDGE_MAX_TOKENS", "16384"))

    validator_system = """You are the Validator Agent in the Recursive language model powered text to CAD generation system.
Your job is to rigorously and objectively evaluate if the resulting geometric metrics and three-view renders fulfill the original user request.

You will receive:
1. The original user prompt.
2. The exact geometric measurements from the OCCT CAD kernel (volume, bounding box, face/edge counts).
3. A rendered image of the generated part showing THREE views side by side:
...
- Features that are clearly absent in the render
...
The "failure_category" field MUST be one of the following string literals (or null if "passed" is true):
- "primitive_gap": The CAD library is missing a needed primitive or parameterization required to create the part.
- "geometry_invalidity": The generated plan compiled but resulted in a non-manifold body or OpenCASCADE kernel error.
- "visual_mismatch": Visual shape looks incorrect or completely different from the requested form, or is missing features.
- "translation_drift": A mismatch between the CAD solid model (STEP) and the output code/parameter representation, or scaling/unit issues.
- "verifier_miss": The validator itself failed to parse, timed out, or made a reasoning error.
- "user_ambiguity": The prompt itself was too vague or contradictory to implement.
...
Output ONLY valid JSON, no other text."""
    ...
    def _extract_json(response: str) -> dict:
        ...
        return {
            ...
            "failure_category": "verifier_miss",
        }
    ...
    try:
        ...
        return {
            ...
            "failure_category": verdict.get("failure_category", None if verdict.get("passed", False) else "verifier_miss"),
            "render_png": render_png,
        }
