 ...
 *{{box-sizing:border-box}}
 body{{font-family:system-ui,Arial,sans-serif;max-width:1000px;width:100%;margin:0 auto;
   padding:0 16px 40px;color:#1a1a1a}}
 h1{{font-size:20px}} h2{{font-size:15px;margin-top:22px}} label{{display:block;margin:12px 0 4px;font-weight:600}}
 textarea,input[type=text]{{width:100%;padding:8px;font-size:14px}} textarea{{height:70px}}
 ...
 .banner.t{{background:#e8f0fe;color:#1a3e8c}} .banner.d{{background:#f4f4f4;color:#444}}
 code{{background:#eee;padding:1px 4px}}
 .tablewrap{{overflow-x:auto;-webkit-overflow-scrolling:touch}}
 table{{border-collapse:collapse;width:100%;font-size:13px}}
 td,th{{border:1px solid #ddd;padding:4px 8px;text-align:left}} th{{background:#f4f4f4}}
 .box{{background:#0e0e0e;color:#d6d6d6;padding:10px;border-radius:6px;max-height:340px;overflow:auto;
 ...
 img{{max-width:100%}} .imgs img{{max-width:300px;border:1px solid #ccc;margin:6px;border-radius:4px;vertical-align:top}}
 pre{{background:#f7f7f7;border:1px solid #e3e3e3;padding:10px;border-radius:6px;max-height:360px;overflow:auto;
   font-size:12px;white-space:pre-wrap;word-break:break-word}}
 .nav{{position:sticky;top:0;background:#fff;display:flex;flex-wrap:wrap;gap:8px;align-items:center;
   padding:10px 0;border-bottom:1px solid #eee;margin-bottom:14px;z-index:30}}
 .nav a.btn{{text-decoration:none;background:#f0f0f0;color:#222;padding:6px 12px;border-radius:6px;
   font-size:13px;border:1px solid #ddd}}
 .nav a.btn:hover{{background:#e4e4e4}}
 .badge{{font-size:12px;padding:3px 9px;border-radius:999px}}
 .badge.on{{background:#e6f4ea;color:#137333}} .badge.off{{background:#eee;color:#777}}
 .spacer{{flex:1}}
 .info{{display:inline-block;width:15px;height:15px;line-height:15px;text-align:center;border-radius:50%;
   background:#888;color:#fff;font-size:11px;font-style:normal;font-weight:700;cursor:help;position:relative}}
 .info:hover::after,.info:focus::after{{content:attr(data-tip);position:absolute;left:18px;top:-4px;
   width:260px;max-width:70vw;background:#111;color:#eee;padding:8px;border-radius:6px;font-size:12px;
   font-weight:400;z-index:40;white-space:normal;box-shadow:0 2px 8px rgba(0,0,0,.3)}}
 @media(max-width:600px){{ .imgs img{{max-width:100%}} }}
</style></head><body>{_nav()}{body}</body></html>""").encode("utf-8")
...
def _final_panel(result):
    ...
    if isinstance(plan, dict):
        ...
        if title:
            ...
            if links:
                parts.append("<h2>Download</h2><p>" + " &nbsp; ".join(links) + "</p>")
    return "\n".join(parts)
